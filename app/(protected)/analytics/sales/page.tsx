...
export default async function SalesAnalyticsPage({ searchParams }: SalesAnalyticsPageProps) {
  ...
  const filters = {
       startDate: typeof params.dateFrom === 'string' ? new Date(params.dateFrom) : undefined,
       endDate: typeof params.dateTo === 'string' ? new Date(params.dateTo) : undefined,
       serviceType: typeof params.serviceTypes === 'string' ? params.serviceTypes.split(',')[0] : undefined,
       providerId: typeof params.providers === 'string' ? params.providers.split(',')[0] : undefined,
       period: typeof params.period === 'string' ? params.period : 'monthly',
  };
