...
const where: Prisma.SenderBlacklistWhereInput = {};

if (senderName) where.senderName = { contains: senderName, mode: "insensitive" };
if (isActive !== null && isActive !== undefined) where.isActive = isActive === "true";
if (effectiveDate) where.effectiveDate = { gte: new Date(effectiveDate) };
    ...
    const [blacklist, total] = await Promise.all([
      db.senderBlacklist.findMany({
        ...
        orderBy: [{ lastMatchDate: "desc" }, { createdAt: "desc" }],
        skip,
        take: limit,
      }),
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to update blacklist entry" }, { status: 500 });
  }
}
