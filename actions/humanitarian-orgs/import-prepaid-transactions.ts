...
/**
 * Validate imported data against expected totals
 */
export async function validateImportedData(reportFileId: string) {
  const reportFile = await db.reportFile.findUnique({
  ...
  reportFile.transactions.forEach(tx => {
    const key = tx.serviceName;
  ...
  return {
    reportFile,
    services: Array.from(serviceGroups.values()),
    summary: {
      totalServices: serviceGroups.size,
      totalTransactions: reportFile.transactions.length,
      totalAmount: reportFile.transactions.reduce((sum, tx) => sum + tx.amount, 0),
      totalCount: reportFile.transactions.reduce((sum, tx) => sum + tx.quantity, 0),
    }
  };
}