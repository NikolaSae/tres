...
import { LogEntryItem, LogFiltersState } from "@/lib/types/log-types"; // ✅ Import from types file
...
// ✅ REMOVED: LogEntryItem and LogFiltersState definitions - now imported from types file
...
export default function ProviderLogList({ logRefreshKey }: ProviderLogListProps) {
  ...
  const getStatusBadge = (status: LogStatus) => {
    switch (status) {
      ...
      case "FINISHED":
        // ✅ FIX: Use 'default' or 'outline' instead of 'success'
      ...
      <CardContent className="space-y-4 pt-6">
        ...
        {total > pagination.limit && (
          <Pagination className="mt-4">
            <PaginationContent>
              <PaginationItem>
                {/* ✅ FIX: Remove disabled prop from PaginationPrevious */}
                <PaginationPrevious
                  onClick={() => {
                  ...
                  className={pagination.page === 1 || loading || !!updatingStatus ? 'pointer-events-none opacity-50' : ''}
                />
              </PaginationItem>
              {Array.from({ length: Math.ceil(total / pagination.limit) }).map((_, index) => (
                <PaginationItem key={index}>
                  {/* ✅ FIX: Remove disabled prop from PaginationLink */}
                  <PaginationLink
                    onClick={() => {
                    ...
                    className={loading || !!updatingStatus ? 'pointer-events-none opacity-50' : ''}
                  >
                    {index + 1}
                  </PaginationLink>
                </PaginationItem>
              ...
              <PaginationItem>
                {/* ✅ FIX: Remove disabled prop from PaginationNext */}
                <PaginationNext
                  onClick={() => {
                    if (pagination.page < Math.ceil(total / pagination.limit) && !loading && !updatingStatus) {
                      handlePageChange(Math.min(Math.ceil(total / pagination.limit), pagination.page + 1));
                    }
                  }}
                  className={pagination.page >= Math.ceil(total / pagination.limit) || loading || !!updatingStatus ? 'pointer-events-none opacity-50' : ''}
                />
              </PaginationItem>
            </PaginationContent>
          </Pagination>
