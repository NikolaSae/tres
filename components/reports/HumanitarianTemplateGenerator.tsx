...
import { toast } from '@/hooks/use-toast';interface TemplateGenerationResult {
  success: boolean;
  message: string;
  processed: number;
  errors?: string[];
...
];const YEARS = Array.from({ length: 10 }, (_, i) => new Date().getFullYear() - 2 + i);export function HumanitarianTemplateGenerator() {
  ...
  const [result, setResult] = useState<TemplateGenerationResult | null>(null);  const handleGenerate = async () => {
...
try {
  ...
  if (generateForAll) {
    ...
    result = await generateAllHumanitarianReports(selectedMonth, selectedYear, selectedPaymentType, selectedTemplateType);
  } else {
    // Generate only templates
    result = await generateHumanitarianTemplates(selectedMonth, selectedYear, selectedPaymentType, selectedTemplateType);
  }
